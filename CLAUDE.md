...
- **URL pública:** `https://decks.nextleveltechbr.com/` (domínio próprio; cada deck em `/<slug>/`)
...
- **Domínio próprio** `decks.nextleveltechbr.com` (subdomínio) definido pelo arquivo `CNAME` na raiz — serve direto na raiz do subdomínio, sem prefixo de pasta.

```
DeckNextLevel/
├── CNAME                 → domínio próprio: decks.nextleveltechbr.com
├── nextlevel-logo.png    → asset compartilhado (referenciado como ../nextlevel-logo.png)
└── <slug-cliente>/
    └── index.html        → https://decks.nextleveltechbr.com/<slug-cliente>/
```

**Regra de ouro da URL:** o caminho da pasta é a URL.
`cliente-exemplo-onboarding/index.html` → `https://decks.nextleveltechbr.com/cliente-exemplo-onboarding/`
...
- **Caminhos relativos sempre** (`../nextlevel-logo.png`). Evite caminhos absolutos (`/logo.png`), porque apontam para `decks.nextleveltechbr.com/logo.png`, fora da pasta do deck.
